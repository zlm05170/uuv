...
class SurfaceVessel(Actor):
    def __init__(self, parent : 'Actor' = None, pose : 'Pose' = Pose(), ship_length =1, speed = 1, scene = None):
        super().__init__(parent, pose)
        self.ship_length = ship_length
        self.velocity_xy = SurfaceVessel.get_velocity(speed, self.pose.position[2])
        self.velocity_yaw = 0
    ...
    def get_cr(self, target_ship: 'SurfaceVessel'):
        position_os_ts = target_ship.pose.position[0:2] - self.pose.position[0:2]
        velocity_os_ts = target_ship.velocity_xy - self.velocity_xy
        ...
        d1 = SurfaceVessel.get_colreg_coefficient(bearing)
        d2 = d1 * 2
    ...
    # @classmethod
    def get_distance(self, target_ship: 'SurfaceVessel'):
        position_os_ts = target_ship.pose.position[0:2] - self.pose.position[0:2]
    ...
    def communicate(self):
        ...
        for key, scene_object in self.scene_ref.items():
            if isinstance(scene_object, SurfaceVessel):
