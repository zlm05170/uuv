...
class AStarPlanner:
    ...
    def planning(self, sx, sy, sz, gx, gy, gz):
        ...
        start_node = self.Node(self.calc_xyz_index(sx, self.min_x),
                               self.calc_xyz_index(sy, self.min_y),
                               self.calc_xyz_index(sz, self.min_z), 0.0, -1)
        goal_node = self.Node(self.calc_xyz_index(gx, self.min_x),
                              self.calc_xyz_index(gy, self.min_y),
                              self.calc_xyz_index(gz, self.min_z), 0.0, -1)
        ...
        while 1:
            ...
            if show_animation:  # pragma: no cover
                plt.gca(projection='3d').plot(self.calc_grid_position(current.x, self.min_x),
                         self.calc_grid_position(current.y, self.min_y),
                         self.calc_grid_position(current.z, self.min_z), "xc")
                plt.xlabel('x')
                plt.ylabel('y')
    ...
    def calc_heuristic(n1, n2):
        ...
        r = w * math.hypot(n1.x - n2.x, n1.y - n2.y)
        d = w * math.hypot(r, n1.z - n2.z)
        return d
    ...
    def calc_grid_index(self, node):
        return (node.y - self.min_y) * self.x_width + (node.x - self.min_x) + (node.y - self.min_y) * self.z_width + (node.z - self.min_z) + (node.x - self.min_x) * self.z_width + (node.z - self.min_z)
    ...
    def calc_obstacle_map(self, ox, oy, oz):
        ...
        print("z_width:", self.z_width)

        # obstacle map generation
        self.obstacle_map = [[[False for _ in range(self.y_width)]
                             for _ in range(self.x_width)] for _ in range(self.z_width)]
...
def main():
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.gca(projection='3d').plot(sx, sy, sz, "og")
        plt.gca(projection='3d').plot(gx, gy, gz, "xb")
        plt.grid(True)
        plt.axis("auto")
    #ox, oy, oz = xl, yl, z_grid
