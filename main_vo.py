...
if __name__ == '__main__':
    ...
    scene.update(ship_a = SurfaceVessel_VO(pose=Pose(position=[0.0,0.0,45.0]), ship_length = 10, speed = 5.0, 
                goal = np.array([30.0,50.0]), scene = scene))
    scene.update(ship_b = SurfaceVessel_VO(pose=Pose(position=[30.0,0.0,315.0]), ship_length = 11, speed = 5.0, 
                goal = np.array([0.0,30.0]), scene = scene))
    scene.update(ship_c = SurfaceVessel_VO(pose=Pose(position=[30.0,30.0,225.0]), ship_length = 12, speed = 5.0, 
                goal = np.array([0.0,0.0]), scene = scene))
    scene.update(ship_d = SurfaceVessel_VO(pose=Pose(position=[0.0,50.0,135.0]), ship_length = 13, speed = 5.0, 
                goal = np.array([30.0,0.0]), scene = scene))
    ...
    tra = draw_vo_trajectory(fig, axs[0], v)
